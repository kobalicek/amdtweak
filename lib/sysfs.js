// ============================================================================
// [sysfs.js]
// A minimalist interface for comfortable sysfs access.
//
...
const sysfs = Object.create(null);
...
sysfs.filterInPlace = filterInPlace;
...
sysfs.readFile = readFile;

function readString(fileName) {
  return readFile(fileName, "UTF-8") || "";
}
sysfs.readString = readString;
...
sysfs.parseList = parseList;

function readList(fileName) {
  const text = readString(fileName);
  return text ? parseList(text, 0) : null;
}
sysfs.readList = readList;
...
sysfs.parseProperties = parseProperties;

function readProperties(fileName) {
  const text = readString(fileName);
  return text ? parseProperties(text, 0) : null;
}
sysfs.readProperties = readProperties;
