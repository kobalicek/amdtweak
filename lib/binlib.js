  ...
  $decomposeStruct(buf, offset, type) {
    ...
    for (var i = 0; i < members.length; i++) {
      ...
      if (flags & Flags.Primitive) {
        ...
        var subType = m.$ref;
        var count = (typeof m.$length === "number") ? m.$length :
                    (typeof m.$length === "string") ? obj[m.$length] || 0 : 0;

        // Indicates to decompose the array by using a specific version of `$ref`.
        if (m.$version) {
          // TODO: Should be a bit better.
          const fullType = subType.$name + "$" + obj[m.$version];
          subType = this.$getType(fullType);
          if (!subType) {
            this.$log(`${type.$name}.${m.$name}: Couldn't get a versioned array entry type '${fullType}'`);
            continue;
          }
        }
        ...
        this.$log(`${type.$name}.${m.$name}: Unhandled type '${m.$type.$name}'`);
      }
    }
