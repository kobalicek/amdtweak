...
//   * Allow to update data back from JS object to the original Buffer.
//
...
//   * Utf8    - String of fixed size, read as UTF-8 string.
...
const FLAGS = freeze({
  ...
  UTF8           : 0x00001000, // String data as UTF8  (always combined with STRING).
...
function updateFlags(existing, add) {
  ...
  const ENCODING = FLAGS.ASCII | FLAGS.HEX | FLAGS.UTF8;
...
function encodingFromFlags(flags) {
  return (flags & FLAGS.HEX)  ? "hex"  :
         (flags & FLAGS.UTF8) ? "utf8" : "ascii";
}
...
const Utf8  = binlib.Utf8  = freeze(new Type({ $name: "Utf8" , $size: 0, $flags: FLAGS.STRING       | FLAGS.UTF8     }));
  ...
  switch (flags & FLAGS_PRIMITIVE_ALL) {
    ...
    case FLAGS_PRIMITIVE_LE | FLAGS.SIGNED:
      switch (size) {
        ...
        case 3: return (buffer.readUInt8(offset + 0)      ) +
                       (buffer.readUInt8(offset + 1) <<  8) +
                       (buffer.readInt8 (offset + 2) << 16) ;
      }
      break;
    ...
    case FLAGS_PRIMITIVE_BE | FLAGS.SIGNED:
      switch (size) {
        case 1: return buffer.readInt8(offset);
        case 2: return buffer.readInt16BE(offset);
        case 4: return buffer.readInt32BE(offset);

        case 3: return (buffer.readInt8 (offset + 0) << 16) +
                       (buffer.readUInt8(offset + 1) <<  8) +
    ...
    case FLAGS_PRIMITIVE_LE | FLAGS.UNSIGNED:
      switch (size) {
        ...
        case 2: return buffer.readUInt16LE(offset);
        case 4: return buffer.readUInt32LE(offset);
    ...
    case FLAGS_PRIMITIVE_BE | FLAGS.UNSIGNED:
      switch (size) {
        case 1: return buffer.readUInt8(offset);
        case 2: return buffer.readUInt16BE(offset);
        case 4: return buffer.readUInt32BE(offset);

        case 3: return (buffer.readUInt8(offset + 0) << 16) +
                       (buffer.readUInt8(offset + 1) <<  8) +
                       (buffer.readUInt8(offset + 2)      ) ;
      }
      break;
    ...
    case FLAGS_PRIMITIVE_LE | FLAGS.FLOAT:
      switch (size) {
        ...
        case 8: return buffer.readDoubleLE(offset);
      }
      break;

    case FLAGS_PRIMITIVE_BE | FLAGS.FLOAT:
      switch (size) {
        case 4: return buffer.readFloatBE(offset);
        case 8: return buffer.readDoubleBE(offset);
      }
      break;
  }
...
function readStringValue(buffer, offset, size, flags) {
  ...
  if (encoding === "hex") {
    return buffer.toString("hex", offset, offset + size);
  }
  else {
    var str = buffer.toString(encoding, offset, offset + size);
    var len = str.length;
...
function writePrimitiveValue(buffer, offset, size, flags, value) {
  // console.log(`WRITE [${offset}] of size '${size}' to '${value}'`);
  ...
  switch (flags & FLAGS_PRIMITIVE_ALL) {
    ...
    case FLAGS_PRIMITIVE_LE | FLAGS.UNSIGNED:
      switch (size) {
        ...
        case 3:
          buffer.writeUInt8((value      ) & 0xFF, offset + 0);
    ...
    case FLAGS_PRIMITIVE_BE | FLAGS.UNSIGNED:
      switch (size) {
        ...
        case 4:
          buffer.writeUInt32BE(value & 0xFFFFFFFF, offset);
          return;

        case 3:
          buffer.writeUInt8((value >> 16) & 0xFF, offset + 0);
          buffer.writeUInt8((value >>  8) & 0xFF, offset + 1);
...
function writeStringValue(buffer, offset, size, flags, value) {
  ...
  if (encoding === "hex") {
    buffer.write(value, offset, size, encoding);
  ...
  if (remain)
    buffer.fill(0, offset + nBytes, offset + nBytes + remain);
}
...
class DataAccess {
  constructor(context, opt) {
    ...
    const length    = typeof opt.length   === "number"   ? opt.length   : buffer.length;
    ...
    this.buffer     = buffer;  // Buffer instance we work with.
    this.start      = start;   // Buffer window (start position).
    this.end        = end;     // Buffer window (end position).
...
class DataReader extends DataAccess {
  ...
  readMember(type, object, member) {
    ...
    if (flags & FLAGS.PRIMITIVE) {
      ...
      return readStringValue(this.buffer, offset, size, flags);
    }
    else if (flags & FLAGS.ARRAY) {
      var ref = member.$ref;
      ...
      if (member.$version) {
        ...
        if (!ref) {
          this.messages.push(`${type.$name}.${member.$name}: Couldn't get a versioned array entry type '${fullType}'`);
          return undefined;
        }
      }

      var array = [];
      var subOffset = offset + member.$offset;
...
class DataUpdater extends DataAccess {
  constructor(context, buffer, start, end, options) {
    super(context, buffer, start, end, options);
  }
