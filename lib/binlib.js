...
function dataViewOf(any) {
  ...
  if (any instanceof ArrayBuffer) {
    ...
    // If this is a node's Buffer then we must use the offset as
    // well as node would normally use parts of larger buffers.
    if (typeof any.offset === "number")
      byteOffset = any.offset;
  }

  if (arrayBuffer === null || !(arrayBuffer instanceof ArrayBuffer))
    throw TypeError(`Couldn't recognize the input buffer type`);

  return new DataView(arrayBuffer, byteOffset);
}
